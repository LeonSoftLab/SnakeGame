...
class Game:
    ...
    def check(self):
        # Check head position
        return self._snake.head["x"] < 0 or self._snake.head["x"] > WIDTH \
                or self._snake.head["y"] < 0 or self._snake.head["y"] > HEIGHT
    ...
    def start_game(self):
        ...
        self._snake = Snake(self._screen,
                            ...
                            SIZE_IMG,
                            VECTORS[randint(0, 3)])
        while self._snake.check_pos(x, y):
            x, y = gen_x_y()
        self._food = Food(self._screen, x, y, RED, SIZE_IMG)
        self._running = True
        ...
        while self._running:
            ...
            if self._food.check(**self._snake.head):
                ...
                while self._snake.check_pos(x, y):
                    x, y = gen_x_y()
                self._food.regen(x, y)
                self._score += 1
            pygame.time.wait(800 // (self._score + 1))
